...
def return_amount_in_rubles_current_rate(transaction: dict) -> Union[float, None]:
    """
    ...
    try:
        ...
        return
    if transaction["operationAmount"]["currency"]["code"] != 'USD' and transaction["operationAmount"]["currency"]["code"] != 'EUR':
        print(f'Неизвестный код валюты {transaction["operationAmount"]["currency"]["code"]} - конвертация невозможна')
        return
    ...
    if response.status_code == 200:
        ...
        print(f'Ошибка {response.status_code} при попытке получения курса валюты')
        return
